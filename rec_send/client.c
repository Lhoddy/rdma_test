...
/** 
the same as on_connect 
diff: post rece  here 
      post send on_connect
*/
void post_receives(struct connection *conn)  
{
  ...
  TEST_NZ(ibv_post_recv(conn->qp, &wr, &bad_wr));    //启动一次CQ
}
...
int on_addr_resolved(struct rdma_cm_id *id)
{
  ...
  post_receives(conn);     //地址解析完成就发送receive
...
void on_completion(struct ibv_wc *wc)
{
  ...
  if (++conn->num_completions == 2)    //如果完成了两个操作，则断开连接
...
int on_connection(void *context)  //在此之前发送的receive已经被消耗，数据接受完成;做client本次连接要做的事
{
  ...
  TEST_NZ(ibv_post_send(conn->qp, &wr, &bad_wr));     //连接完成之后发送send，此即是RDMA_SEND，启动一次CQ
